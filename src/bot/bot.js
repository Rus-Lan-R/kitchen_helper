...
const botListiner = async () => {
			...
			case "/healthlabel":
				bot.onText(/\/healthlabel (.+)/, async (msg, match) => {
					try {
						...
						bot.sendMessage(
							chatId,
							`Find ${response.hits.length} ${msg.text} recepi`,
							makeButtonForChoseRecipe(response.hits),
						);
					} catch (error) {
	...
	bot.on("callback_query", (query) => {
		...
		if (query.data === "details") {
			response.hits[currentRecipe].recipe.ingredients.forEach((el) =>
				bot.sendMessage(chatId, makeFullRecipe(el)),
			);
...
const makeButtonForChoseRecipe = (arrRecipe) => {
	let arr = arrRecipe.map((el, index) => [
		{ text: `${el.recipe.label}`, callback_data: `${index}` },
	]);
	let obj = {
		reply_markup: {
			resize_keyboard: true,
...
const makeRecipeMessage = (recipe) => {
	let ingredients = recipe.ingredientLines.reduce(
		(acc, el, index) => acc + `${index + 1}) ` + el + "\n",
		"",
	);
	return `
